...
POSSIBLE_MEASURES=(RAcc RelativeF1 RelativeFBeta)
...
if [ "$COMPARE_WITH_COMPLETE" -eq "0" ]; then
   ./boxplot.py "results/""$ARRF_NAME""_""$TAKE_ATTRIBUTE""_"$MIN_SUP"_""$TOPK_COUNT""_""$MAX_REDUNDANCY""_""$REDUNDANCY_MEASURE_PARAM" "$MEASURE_PARAM"
   echo -n ""
else
